...
def _build_translation_messages(english_text: str, strict: bool = False) -> list[dict]:
    base_instruction = (
        ...
        "Do not explain, do not include labels, and do not include markdown."
    )
    strict_instruction = (
...
#     # Extract bold blocks like **Hello.** **Maka ...**
#     bold_blocks = [b.strip() for b in re.findall(r"\*\*(.*?)\*\*", text, flags=re.DOTALL)]

#     # Prefer the longest non-greeting bold block
...
#     # Fallback: last meaningful non-empty line, skipping greetings
#     lines = [line.strip() for line in text.splitlines() if line.strip()]
